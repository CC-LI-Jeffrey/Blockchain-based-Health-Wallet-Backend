...
async function main() {
    ...
    console.log('\n[1/6] Compiling VaccineVerify.circom...');
    ...
    if (fs.existsSync(ptauPath)) {
        console.log('\n[2/6] Reusing existing pot12_final.ptau (from AgeVerify setup)');
    ...
    run('snarkjs zkey export solidityverifier VaccineVerify_final.zkey VaccineVerifier.sol');
    const srcVerifier = path.join(CIRCUITS_DIR, 'VaccineVerifier.sol');
    const dstVerifier = path.join(CONTRACTS_DIR, 'VaccineVerifier.sol');
    if (fs.existsSync(srcVerifier)) {
        fs.copyFileSync(srcVerifier, dstVerifier);
        console.log(`✅ VaccineVerifier.sol → contracts/VaccineVerifier.sol`);
    }

    // Step 6: Copy circuit files for Android
    console.log('\n[6/6] Copying circuit files for Android...');
    ...
    console.log('==============================================');
    console.log('1. Open contracts/VaccineVerifier.sol');
    console.log('   Copy all curve constants into contracts/VaccineVerifyExtension.sol');
    console.log('   (replace the PLACEHOLDER_* values in the Groth16Verifier section)');
    console.log('2. Copy the two asset files listed above into the Android assets folder');
    console.log('3. Deploy VaccineVerifyExtension.sol to Sepolia');
    console.log('4. Update VACCINE_VERIFY_CONTRACT address in BlockchainService.kt');
}
